...
departements = departements.map(d => {
  ...
  return {
    departement_num: d.num_dep.toString(),
    departement_nom: d.dep_name,
    departement_region: d.region_name,
    niveau_gravite_max: depZones.length > 0 ? getNiveauInversed(max(depZones.map(z => getNiveau(z.niveauAlerte)))) : null
  }
});
...
export function computeZoneApplicable({lon, lat, codeCommune}) {
  ...
  if (hasLonLat) {
    lonLatZones = searchZonesByLonLat({lon, lat})
