...
function computeZoneScore(zone, isIntersecting, typePrio) {
...
export function computeZoneApplicable({lon, lat, codeCommune}) {
  ...
  if (zonesWithMaxScore.length > 1 && !hasLonLat) {
    throw createError(404, 'Veuillez renseigner une adresse pour préciser la réglementation applicable')
  }
