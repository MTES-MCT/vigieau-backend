...
export function searchZonesByCommune(commune) {
...
export function searchZone({lon, lat}) {
  ...
  if (zones.length === 0) {
    throw createError(404, 'Aucune zone d’alerte en vigueur à cet endroit.')
  }
  ...
  const arretes = chain(zones).map(z => z.arrete).uniqBy('idArrete').value()
