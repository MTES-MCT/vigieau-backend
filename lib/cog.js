...
async function readDepartements() {
  const dataDepartements = JSON.parse(await readFile(
    require.resolve('@etalab/decoupage-administratif/data/departements.json'),
    {encoding: 'utf8'}
  ))
  const dataRegions = JSON.parse(await readFile(
    require.resolve('@etalab/decoupage-administratif/data/regions.json'),
    {encoding: 'utf8'}
  ))

  return dataDepartements
    .filter(d => d.zone !== 'com')
    .map(d => {
      d.region = dataRegions.find(r => r.code === d.region)
      return d
    })
}

const departements = await readDepartements()
