...
import {sendMessage} from './util/webhook.js'
import {sendMail, computeUnsubscribeUrl} from './util/sendmail.js'
...
export async function subscribe(payload, ipAddress) {
  ...
  const isCreated = upsertedCount === 1

  if (isCreated) {
    await sendMail(29, subscription.email, {
      city: getCommune(subscription.commune).nom,
      address: subscription.libelleLocalisation,
      unsubscribeUrl: computeUnsubscribeUrl(subscription.email)
    })

    await sendMessage(`Nouvelle inscription aux alertes VigiEau sur la commune de **${getCommune(subscription.commune).nom}** (${subscription.profil})`)
  }
